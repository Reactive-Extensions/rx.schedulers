...
var HistoricalScheduler = require('../historicalscheduler');

function arrayEquals(t, first, second) {
  ...
  for (var i = 0, len = first.length; i < len; i++) {
    ...
    if (f.equals && s.equals) {
      t.ok(f.equals(s), 'first should equal the second');
    } else {
      t.ok(f === s, 'first should equal the second');
    }
  }
}
